    ...
    function(
        ...
        runA = function() {
            ...
            test('surfaceDistR_along_azimuth(phiR_start, thetaR_start, phiR_end, approximationPrecision, maxRecursionDepth) should return an approximation of the distance along azimuth between two points. Direction is determined by (point_end - point_start)', function() {
                equal(Math.floor(surface.surfaceDistR_along_azimuth(0,0,Math.PI,       undefined, false)) == 314, true, 'X should be ~Pi*100 (314.159265359).');
                equal(surface.surfaceDistR_along_azimuth(0,0,Math.PI*2,     undefined, false), 624.2890304516104, 'X should be ~Pi*2*100 (628.318530718).');
                // DOESN'T WORK AS EXPECTED: equal(surface.surfaceDistR_along_azimuth(0,0,24.545829796022296, undefined, false), 42, 'X should be ~?.');
                equal(surface.surfaceDistR_along_azimuth(0,0,0,             undefined, false), 0, 'X should be 0.');
                equal(surface.surfaceDistR_along_azimuth(0,0,-Math.PI/2,    undefined, false), 156.0722576129026, 'X should be ~Pi/2*100 (157.079632679).');
                equal(surface.surfaceDistR_along_azimuth(0,0,-Math.PI*1.5,  undefined, false), 464.45548360713974, 'X should be ~Pi*1.5*100 (471.238898038).');
                equal(surface.surfaceDistR_along_azimuth(0,0,-Math.PI*1.5,  undefined, false), 464.45548360713974, 'X should be ~Pi*1.5*100 (471.238898038).');
              ...
              test('surfaceDistR_along_polar(phiR_start, thetaR_start, thetaR_end, approximationPrecision, maxRecursionDepth) should return an approximation of the distance along polar, between two points. Direction is determined by (point_end - point_start)', function() {
                equal(surface.surfaceDistR_along_polar(0,0,Math.PI, undefined, false), 312.1445152258052,
                    'X should be ~Pi*100 (314.159265359).');
                equal(surface.surfaceDistR_along_polar(0,0,Math.PI*2,     undefined, false), 624.2890304516104,
                    'X should be ~Pi*2*100 (628.318530718).');
                equal(surface.surfaceDistR_along_polar(0,0,0,             undefined, false), 0, 'X should be 0.');
                equal(surface.surfaceDistR_along_polar(0,0,-Math.PI/2,    undefined, false), 156.0722576129026,
                    'X should be ~Pi/2*100 (157.079632679).');
                equal(surface.surfaceDistR_along_polar(0,0,-Math.PI*1.5,  undefined, false), 464.45548360713974, 'X should be ~Pi*1.5*100 (471.238898038).');
            });
...
//                equal(surface.horizontalSurfaceProjection(0, 0, 0, 1, false), 0,
//                      'X should be ~0.');
//                equal(surface.horizontalSurfaceProjection(0,0,Math.PI*50, 1, false), 1.5758657128552445,
//                      'X should be ~Pi/2 (1.57079632679).');
//                equal(surface.horizontalSurfaceProjection(0,0,Math.PI*100, 1, false), 3.156866656323774,
//                      'X should be ~Pi (3.14159265359).');
//                equal(surface.horizontalSurfaceProjection(0,0,Math.PI*200, 1, false), 6.318935407868947,
...
//                equal(surface.horizontalSurfaceProjection(0,0,Math.PI,       0.01,false), 314.0331156954753, 'X should be ~Pi*100 (314.159265359).');
//            });
